# Deploying Ops Autopilot to Alibaba Cloud
...
2. Create a database (e.g. `ops_autopilot`) and a user with access to it.
...
4. Build the connection string using the **psycopg3** driver prefix (not bare
   ...
   ```
   DATABASE_URL=postgresql+psycopg://<user>:<password>@<rds-host>:5432/ops_autopilot
   ```

## 2. Create an OSS bucket

1. Create a bucket (e.g. `ops-autopilot-payloads`) in the Alibaba Cloud OSS console, in
...
3. Set:
   ...
   OSS_BUCKET_NAME=ops-autopilot-payloads
   ```
...
```bash
cd backend
docker build -t ops-autopilot:latest .

# Push to Alibaba Cloud Container Registry (ACR)
docker tag ops-autopilot:latest registry.<region>.aliyuncs.com/<namespace>/ops-autopilot:latest
docker login registry.<region>.aliyuncs.com
docker push registry.<region>.aliyuncs.com/<namespace>/ops-autopilot:latest
```
...
2. Pull and run the image, passing all required env vars:
   ```bash
   docker run -d --name ops-autopilot -p 80:8000 \
     -e DASHSCOPE_API_KEY=... \
     ...
     -e OSS_ENDPOINT=... \
     -e OSS_BUCKET_NAME=... \
     registry.<region>.aliyuncs.com/<namespace>/ops-autopilot:latest
   ```
...
```bash
docker build -t ops-autopilot:test .
docker run -d --name ops-autopilot-test -p 18000:8000 \
  -e DASHSCOPE_API_KEY=dummy \
  -e DATABASE_URL=sqlite:////tmp/ops_autopilot.db \
  ops-autopilot:test
curl localhost:18000/health   # -> {"status":"ok"}
curl localhost:18000/dashboard
docker stop ops-autopilot-test && docker rm ops-autopilot-test
```
